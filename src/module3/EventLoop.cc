...
{
    _onConnectionCb = move(onConnectionCb);
}
...
void EventLoop::setPendingCallBack(PendingCallBack &&onPendingCb)
{
    ...
    _pendingCbs.push_back(move(onPendingCb));
}
...
void EventLoop::waitEpoll()
{
    ...
    else // 正常情况：readyNum > 0
    {
        ...
        for (int idx = 0; idx < readyNum; ++idx)
        {
            ...
            {
                if (_eventList[idx].events & EPOLLIN) // 确认事件类型是否为读就绪
                {
                    handleNewConnection();
                }
            }
