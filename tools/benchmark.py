...
VALID_RESPONSE_IDS = {1: {100, 404}, 2: {200, 404}}
...
async def exchange(session: Session, payload: bytes, msg_id: int, max_response_bytes: int) -> float:
    ...
    if response.get("msgID") not in VALID_RESPONSE_IDS[msg_id]:
        raise ValueError(f"unexpected response msgID: {response.get('msgID')!r}")
    ...
    try:
        for _ in range(args.warmup):
            await asyncio.wait_for(
                exchange(session, payload, args.msg_id, args.max_response_bytes), args.timeout
            )
        return session
...
async def measure_session(session: Session, args: argparse.Namespace, payload: bytes, gate: asyncio.Barrier) -> WorkerResult:
    result = WorkerResult()
    try:
        await gate.wait()
...
def print_report(args: argparse.Namespace, elapsed_s: float, results: Sequence[WorkerResult]) -> None:
...
async def run(args: argparse.Namespace) -> int:
    ...
    workers = [asyncio.create_task(measure_session(session, args, payload, gate)) for session in sessions]
    await gate.wait()
    started = time.perf_counter()
    results = await asyncio.gather(*workers)
    print_report(args, time.perf_counter() - started, results)
